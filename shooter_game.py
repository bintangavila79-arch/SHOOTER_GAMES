...
background = transform.scale(image.load('lava.jpg'), (700, 500))
...
def gererate_enemies(number=5, asteroid_number=2):
    for i in range(number):
        enemy = Enemy('ufo.png', randint(40, 660), 0, randint(1, 3))
        enemies.add(enemy)
    for i in range(asteroid_number):
        asteroid = Enemy('asteroid.png', randint(40, 660), 0, randint(1, 2))
...
while run:
    ...
    if not finish:
        ...
        if reload_time == True:
            ...
            if now_time - last_reload_time < 2: #before 3 seconds are over, display reload message
